trigger SCAction_SetValuesBeforeInsert on SC_Action__c (before insert) {
     ...
     for (SC_Action__c sca : Trigger.new)
     {
         if(sca.Opportunity__c != null){
              ...
              List<Opportunity> oList = [SELECT 
                   Tech_Assessment__c, Id, AccountId
         ...
         if(sca.Solution_Assessment__c != null){
             if(sca.Account__c == null && sca.Opportunity__c == null){
                List<Profile_Qualification__c> pqList = [SELECT Id, Account__c, Opportunity__c
                   FROM Profile_Qualification__c where Id=:sca.Solution_Assessment__c ORDER By LastModifiedDate DESC LIMIT 1];
                 if(pqList.size()>0)
                 {
                     sca.Opportunity__c = pqList[0].Opportunity__c;
                     sca.Account__c = pqList[0].Account__c;       
                 }
             }
