trigger UpdateOppSummaryAfterUpdate on Mutual_Close_Plan__c (after update) 
{
    List<Mutual_Close_Plan__c> mcPlans = new List<Mutual_Close_Plan__c>();
    for(Mutual_Close_Plan__c mcp:Trigger.new)
    { 
           Mutual_Close_Plan__c mcpOld = Trigger.oldMap.get(mcp.Id);
           if (mcp.Is_Completed__c != mcpOld.Is_Completed__c)
           {
